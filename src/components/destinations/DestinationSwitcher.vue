...
<template>
  <div class="flex flex-col gap-8">
    ...
    <div
      class="mt-8 flex flex-col items-center justify-center gap-6 text-center xl:items-start xl:text-left"
    >
      <DestinationSelect v-model="dest" />
      <h1
        class="text-preset-2 text-[3.5rem] uppercase md:text-[5rem] xl:text-[6rem]"
      >
        {{ (tl as any)[alts[dest]] }}
      </h1>
      <p
        class="text-preset-9 text-center text-[15px] text-blue-300 md:max-w-[32.125rem] md:text-base xl:max-w-[28rem] xl:text-left xl:text-lg"
      >
